...
def display_class_names(dataset: str, labels: Sequence[object]) -> List[str]:
    if dataset == "pamap2":
        return [PAMAP2_ACTIVITY_MAP.get(int(x), str(x)) for x in labels]
...
def pamap2_feature_cols(feature_set: str) -> List[str]:
    cols = []
    if feature_set.endswith("_hr"):
        cols.append("heart_rate")
    for pos in ["hand", "chest", "ankle"]:
        cols += [f"{pos}_acc16_x", f"{pos}_acc16_y", f"{pos}_acc16_z"]
        if feature_set in {"acc16_gyro", "acc16_gyro_hr", "allimu_hr"}:
...
def find_pamap2_root(raw_root: Path) -> Path:
    candidates = [
        raw_root / "pamap2" / "PAMAP2_Dataset",
        raw_root / "PAMAP2_Dataset",
        raw_root / "pamap2",
    ]
    for c in candidates:
        if c.exists():
    ...
    raise FileNotFoundError(
        "PAMAP2 root folder not found. Expected one of:\n" +
...
def load_pamap2_dataset(raw_root: Path, task: str, feature_set: str, window: int, step: int, sessions: str = "all") -> Tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame, List[str], List[str]]:
    root = find_pamap2_root(raw_root)
    session_dirs = pamap2_session_dirs(root, sessions)
    files: List[Path] = []
    for session_dir in session_dirs:
        files.extend(sorted(session_dir.glob("subject*.dat")))
    if not files:
        raise FileNotFoundError("No subject*.dat files found in: " + ", ".join(str(p) for p in session_dirs))
    ...
    present_ids = set(int(x) for x in np.unique(labels).tolist())
    if 0 in present_ids:
    ...
    if task == "all18":
        ...
        if missing:
            raise RuntimeError(
                ...
                + ". Use --pamap2-sessions all so Protocol + Optional data are included."
            )
    meta = pd.DataFrame(meta_rows)
    meta.attrs["timestamp_audit_rows"] = timestamp_audit_rows
...
def _balanced_accuracy_without_sklearn_warning(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    recalls = np.divide(
        np.diag(cm),
        denom,
        out=np.zeros(labels.size, dtype=np.float64),
        where=denom != 0,
    )
...
def metrics_dict(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    return {
        ...
        "balanced_accuracy": _balanced_accuracy_without_sklearn_warning(y_true, y_pred),
...
def run_loso_experiment(X: np.ndarray, y_raw: np.ndarray, subjects: np.ndarray, meta: pd.DataFrame, class_names_hint: Sequence[str], feature_names: Sequence[str], out_dir: Path, include_xgb: bool, use_cuda: bool, fast: bool, dataset: str) -> None:
    ensure_dir(out_dir)
    le = LabelEncoder()
    ...
    for model_name, base_model in models.items():
        ...
        for train_idx, test_idx, fold_subject in loso_indices(subjects):
            ...
            t0 = time.time()
            try:
                pred, label_encoding_note = fit_predict_fold_model(model, model_name, X[train_idx], y[train_idx], X[test_idx])
            ...
            fold_rows.append({"model": model_name, "fold_subject": fold_subject, "n_train": len(train_idx), "n_test": len(test_idx), "fit_predict_sec": elapsed, "label_encoding_note": label_encoding_note, **m})
        ...
        report = classification_report(y_true_arr, y_pred_arr, labels=np.arange(len(labels)), target_names=labels_display, output_dict=True, zero_division=0)
...
def main() -> None:
    ...
    for ds in datasets:
        ...
        if ds == "pamap2":
            ...
            ds_out = out_base / f"pamap2_{args.pamap2_task}_{args.pamap2_sessions}_{args.pamap2_feature_set}_w{args.pamap2_window}_s{args.pamap2_step}"
        else:
        ...
        ).to_csv(ds_out / "pamap2_timestamp_audit.csv", index=False)
        dataset_info = {
            "dataset": ds,
            ...
            "timestamp_conflict_audit": "pamap2_timestamp_audit.csv" if ds == "pamap2" else "",
            "timestamp_conflict_policy": "Within one source recording, duplicate timestamps with different activity IDs are dropped before windowing; duplicate timestamps with the same activity ID keep the first row. Protocol and Optional files are separate recordings and windows never cross source files." if ds == "pamap2" else "",
        }
        with open(ds_out / "dataset_manifest.json", "w", encoding="utf-8") as f:
            json.dump(dataset_info, f, indent=2)
        run_loso_experiment(X, y, subjects, meta, class_names, feature_names, ds_out, include_xgb=args.include_xgb, use_cuda=args.use_cuda, fast=args.fast, dataset=ds)
